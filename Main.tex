...
\usepackage[T1]{fontenc}                         % Para las fuentes (T1 encoding)
...
\setstretch{1.3}                                                    % Interlineado de uno y medio. 1.6 es interlineado doble.
...
% Paquetes para gráficos, figuras y tablas
...
\usepackage{adjustbox}                          % Permite más versatilidad para cosas como includegraphics.
\usepackage{float}                              % Permite poner figuras en cualquier lugar.
\usepackage{wrapfig}                            % Permite poner figuras con texto alrededor.
...
% Organización del documento y referencias internas
...
\usepackage{xparse}                             % Ayuda en la creación de nuevos comandos
...
% Configuración de listings como fallback para minted
% \usepackage{minted}                             % Permite darle formato al código que se muestre.
% Fallback: use listings (no -shell-escape required). If you prefer minted, uncomment the minted line above and compile with -shell-escape.
\usepackage{listings}
\usepackage{environ}

% Define un estilo para listings y un entorno tipo minted
...
\lstset{style=myListing}

\NewEnviron{minted}[1]{%
  \lstset{style=myListing, language=#1}%
  \begin{lstlisting}
\BODY
  \end{lstlisting}
}

% Enlaces clickeables y estilo de URL
...
\setlength{\headheight}{15pt}               % Altura suficiente para fancyhdr
\addtolength{\topmargin}{-2.5pt}            % Compensa la nueva altura de cabecera
...
\hypersetup{  %Define la forma en que se verán las cosas clickeables.
  	...
  	colorlinks = true,                          % Elimina las cajas alrededor de lo clickeable y lo reemplaza por palabras a color.
}

\begin{document}
\pagenumbering{roman}                       % Usa numeración romana desde la portada
